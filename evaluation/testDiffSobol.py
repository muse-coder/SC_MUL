...
#########################################################                       
    ...
    bitstreamMode = 1 # 1->tow sobol  0-> sobol and unary    	###
    errorMode = 1 # 1->MRED  0-> NMED
#########################################################
    ...
    count = 0
    for num_1 in range(1,dataRange[1]):
        for num_2 in range(1,dataRange[1]):
            count+=1        
            num_1 = 1
            num_2 = 1
            ...
            for i in range(len(sobolGroups)):
                isc_res=scaled_mul(
                    ...
                    sobolWidth= sobolWidth,dataWidth=  dataWidth,mode=bitstreamMode
                )
                ED = abs(exact_res-isc_res)
                # if(ED>pow(2,dataWidth)*2):
                #     print("1")
                if(errorMode):
                    error= ED/exact_res
                    mredGroup[i]+=(error)
    ...
    if(errorMode):
        mredGroup = [mredGroup [i] /(count) for i in range(len(mredGroup))]
    else:
        mredGroup = [mredGroup [i] /(count) for i in range(len(mredGroup))]
        # mredGroup = [mredGroup [i] /(maxED[i]*count) for i in range(len(mredGroup))]

    for i in range(len(mredGroup)):
        
        print("%.8lf"%(mredGroup[i])+";",end=' ')
    averageMRED = statistics.mean(mredGroup)
    # print("\naverage MRED = %.4lf"%(averageMRED)+"%")
